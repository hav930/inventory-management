    ...
    <div v-else>
      ...
      <div v-if="submittedOrders.length > 0" class="card">
        <div class="card-header">
          <h3 class="card-title">{{ t('orders.submittedOrdersTitle') }} ({{ submittedOrders.length }})</h3>
        </div>
          ...
          <table class="orders-table">
            ...
            <tbody>
              <tr v-for="order in submittedOrders" :key="order.id">
                ...
                <td class="col-lead-time">{{ orderLeadTime(order) }} {{ t('restocking.days') }}</td>
...
export default {
  ...
  setup() {
    ...
    const { submittedOrders, loaded: submittedLoaded, loadSubmitted } = useSubmittedOrders()
    ...
    // Submitted orders persist across renders via the shared composable; only
    // fetch once per session — Restocking.vue updates the ref directly on submit.
    const orderLeadTime = (order) => {
      if (!order.items || order.items.length === 0) return 0
      return Math.max(...order.items.map(i => i.lead_time_days || 0))
    }

    onMounted(async () => {
      await loadOrders()
      if (!submittedLoaded.value) await loadSubmitted()
    })
